...
func _ready() -> void:
	...
	# Formations
	var line_5 := _formation(Formation.Shape.LINE, Vector2(40.0, 0.0))
	var v_shape := _formation(Formation.Shape.V, Vector2(35.0, 25.0))
	var grid_3x2 := _formation(Formation.Shape.GRID, Vector2(40.0, 30.0), 3)
	var circle_6 := _formation(Formation.Shape.CIRCLE, Vector2.ZERO, 1, 50.0)
	var point := _formation(Formation.Shape.POINT)

	# Move styles
	var drift_slow := _move_style(MoveStyle.Type.DRIFT, 80.0, 0.0)
	var drift_fast := _move_style(MoveStyle.Type.DRIFT, 120.0, 0.0)
	var sine_strafe := _move_style(MoveStyle.Type.SINE_STRAFE, 60.0, 0.0, Vector2(50.0, 20.0), 0.8)
	var sine_fast := _move_style(MoveStyle.Type.SINE_STRAFE, 90.0, 0.0, Vector2(60.0, 30.0), 1.2)
	var dive := _move_style(MoveStyle.Type.DIVE_AT_PLAYER, 70.0, 0.0)
	var swoop := _move_style(MoveStyle.Type.SWOOP, 80.0, 0.0, Vector2(60.0, 40.0), 0.6)
	var orbit := _move_style(MoveStyle.Type.ORBIT, 30.0)

	# Rush-follow: fly in fast, then orbit the player at close range
	var rush_follow := _move_style(MoveStyle.Type.RUSH_FOLLOW, 0.0)
	rush_follow.follow_distance = 25.0
	rush_follow.rush_turn = 5.0
	rush_follow.orbit_radius = 8.0
	rush_follow.orbit_speed = 2.0

	var rush_follow_tight := _move_style(MoveStyle.Type.RUSH_FOLLOW, 0.0)
	rush_follow_tight.follow_distance = 18.0
	rush_follow_tight.rush_turn = 6.0
	rush_follow_tight.orbit_radius = 5.0
	rush_follow_tight.orbit_speed = 2.5

	# Patterns (firing)
	var slow_fire := _pattern(2.0, 70.0)
	var med_fire := _pattern(1.2, 90.0)
	var fast_fire := _pattern(0.8, 110.0)
	var no_fire := _pattern(999.0, 0.0) # effectively doesn't shoot
...
func _formation(shape: Formation.Shape, spacing := Vector2(5.0, 3.0),
...
func _move_style(type: MoveStyle.Type, p_speed_z: float = 12.0,
		p_speed_x: float = 0.0, amplitude := Vector2(4.0, 2.0),
		frequency: float = 1.2) -> MoveStyle:
	var m := MoveStyle.new()
	m.type = type
	m.speed_z = p_speed_z
	m.speed_x = p_speed_x
	...
	# RUSH_FOLLOW defaults are set on the resource; callers override after creation
	return m


func _custom_move_style(logic: MoveLogic) -> MoveStyle:
	var m := MoveStyle.new()
	m.type = MoveStyle.Type.CUSTOM
...
func _slot_combat(required_tags: Array = [], excluded_min: int = 0,
